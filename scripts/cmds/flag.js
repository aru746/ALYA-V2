  ...
  onStart: async function ({ api, event, usersData, args }) {
    ...
    if (args[0] && args[0].toLowerCase() === "list") {
      ...
      for (const user of allUsers) {
        if (user.data?.flagWins > 0) {
          ranking.push({
            name: user.name || "Unknown",
            wins: user.data.flagWins
          });
        }
      }
    ...
    try {
      ...
      console.error(error);
      api.sendMessage("❌ Failed to fetch flag.", event.threadID, event.messageID);
    }
  },
...
`✅ | 𝐂𝐨𝐫𝐫𝐞𝐜𝐭 𝐚𝐧𝐬𝐰𝐞𝐫 𝐛𝐚𝐛𝐲! 𝐘𝐨𝐮 𝐞𝐚𝐫𝐧𝐞𝐝 5𝐤 𝐜𝐨𝐢𝐧𝐬 & 251 𝐞𝐱𝐩.`,
      ...
      api.sendMessage(
`🥺 | 𝐖𝐫𝐨𝐧𝐠 𝐀𝐧𝐬𝐰𝐞𝐫 𝐛𝐚𝐛𝐲! 𝐂𝐨𝐫𝐫𝐞𝐜𝐭 𝐚𝐧𝐬𝐰𝐞𝐫: ${flag}`,
        event.threadID,
        event.messageID
      );
    }
