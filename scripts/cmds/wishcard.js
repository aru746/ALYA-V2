module.exports = {
  ...
  onStart: async function ({ api, event, args, usersData }) {
    ...
    let text = args.join(" ")
    if (!text) return api.sendMessage('💢𝐏𝐥𝐞𝐚𝐬𝐞 𝐞𝐧𝐭𝐞𝐫 𝐭𝐡𝐞 𝐜𝐨𝐫𝐫𝐞𝐜𝐭 𝐟𝐨𝐫𝐦𝐚𝐭 [𝐭𝐞𝐱𝐭1 - 𝐭𝐞𝐱𝐭2] ', event.threadID, event.messageID);
    const text1 = text.substr(0, text.indexOf(' - '));
    if (!text1) return api.sendMessage('💢𝐏𝐥𝐞𝐚𝐬𝐞 𝐞𝐧𝐭𝐞𝐫 𝐭𝐡𝐞 𝐜𝐨𝐫𝐫𝐞𝐜𝐭 𝐟𝐨𝐫𝐦𝐚𝐭 [𝐭𝐞𝐱𝐭1 - 𝐭𝐞𝐱𝐭2] ', event.threadID, event.messageID);
    const text2 = text.split(" - ").pop()
    if (!text2) return api.sendMessage('💢𝐏𝐥𝐞𝐚𝐬𝐞 𝐞𝐧𝐭𝐞𝐫 𝐭𝐡𝐞 𝐜𝐨𝐫𝐫𝐞𝐜𝐭 𝐟𝐨𝐫𝐦𝐚𝐭 [𝐭𝐞𝐱𝐭1 - 𝐭𝐞𝐱𝐭2] ', event.threadID, event.messageID);
