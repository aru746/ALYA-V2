...
module.exports = {
  ...
  onStart: async function ({ args, message, usersData, event }) {
    ...
    // --- Cooldown system: 15 games per 7h ---
    const now = Date.now();
    ...
    // Remove plays older than 7h
    history = history.filter(t => now - t < 7 * 60 * 60 * 1000);

    if (history.length >= 15) {
      const firstPlay = history[0];
      const remaining = 7 * 60 * 60 * 1000 - (now - firstPlay);
      const hours = Math.floor(remaining / 3600000);
      const minutes = Math.floor((remaining % 3600000) / 60000);
      const seconds = Math.floor((remaining % 60000) / 1000);
      return message.reply(
        `❌ | You have reached your Sicbo limit (15 plays per 7 hours).\n` +
    ...
    // --- Outcome probabilities (no jackpot) ---
    const winRates = { big: 0.40, small: 0.44 }; 
    const roll = Math.random();
    let outcome;

    if (roll < winRates[betType]) {
      outcome = "win";
    } else {
      outcome = "lose";
    }
