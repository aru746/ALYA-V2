  ...
  onStart: async function ({ api, event, usersData, args }) {
    ...
    if (args[0] && args[0].toLowerCase() === "list") {
      ...
      for (const user of allUsers) {
        if (user.data?.mathWins > 0) {
          ranking.push({
            name: user.name || "Unknown",
            wins: user.data.mathWins
          });
        }
      }
...
𝐑𝐞𝐩𝐥𝐲 𝐰𝐢𝐭𝐡 𝐲𝐨𝐮𝐫 𝐚𝐧𝐬𝐰𝐞𝐫.`
  ...
  onReply: async function ({ event, api, Reply, usersData }) {
    ...
    if (reply === correctAns || reply === correctAns[0]) {
      ...
      await usersData.set(author, {
        ...
        data: {
          ...(userData.data || {}),
          mathWins: (userData.data?.mathWins || 0) + 1
        }
      });
...
`✅ | 𝐂𝐨𝐫𝐫𝐞𝐜𝐭 𝐚𝐧𝐬𝐰𝐞𝐫 𝐛𝐚𝐛𝐲! 𝐘𝐨𝐮 𝐞𝐚𝐫𝐧𝐞𝐝 5𝐤 𝐜𝐨𝐢𝐧𝐬 & 251 𝐞𝐱𝐩.`,
      ...
      return api.sendMessage(
`❌ | 𝐖𝐫𝐨𝐧𝐠 𝐚𝐧𝐬𝐰𝐞𝐫 𝐛𝐚𝐛𝐲! 𝐓𝐡𝐞 𝐂𝐨𝐫𝐫𝐞𝐜𝐭 𝐚𝐧𝐬𝐰𝐞𝐫: ${correctAnswer}`,
        event.threadID,
        event.messageID
      );
    }
