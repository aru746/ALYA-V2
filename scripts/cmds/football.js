...
module.exports = {
  config: {
    name: "football",
    aliases: ["fball", "footqz", "footballqz"],
    version: "2.0",
    author: "MahMUD",
    countDown: 10,
    role: 0,
  ...
  onStart: async function ({ api, event, usersData, args }) {
    ...
    if (args[0] && args[0].toLowerCase() === "list") {
      ...
      for (const user of allUsers) {
        if (user.data?.footballWins > 0) {
          ranking.push({
            name: user.name || "Unknown",
            wins: user.data.footballWins
          });
        }
      }
      ...
      let msg = `⚽ | ${bold("Football Quiz Ranking")}:\n\n`;
  ...
  onReply: async function ({ event, api, Reply, usersData }) {
    ...
    if (userReply === correct || userReply === correct[0]) {
      ...
      await usersData.set(author, {
        ...
        data: {
          ...(userData.data || {}),
          footballWins: (userData.data?.footballWins || 0) + 1
        }
      });
